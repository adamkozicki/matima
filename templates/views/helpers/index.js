...
module.exports = function () {
	...
	* */
	...
	_helpers.galleryUrl = function (galleryUrl, options) {
	return ('/galeria/' + galleryUrl);
	};

	_helpers.imageUrl = function (galleryReference, title, options) {
		var output = '../../photos/' + galleryReference +'/'+ title;
		return new hbs.SafeString(output);
	};
	...
	_helpers.checkIfClient = function(user, options){
		var output = '';

		if(user.isClient == true){
			output += '<button id="changeStatusImage" type="submit" form="imageStatus" class="pswp__button userSettings" title="Zmień status"><span class="glyphicon glyphicon-cog"></span></button>' + 
					  '<button id="removeImage" type="submit" form="imageDelete" class="pswp__button userSettings remove" title="Usuń zdjęcie"><span class="glyphicon glyphicon-remove-sign"></span></button>'
		}else{
			output = ''
		}
	...
	_helpers.checkIfClient2 = function(isClient, state, id, galleryReference, title, options) {
		var output = '';
		if(isClient == true) {
			output += '<p>' + state + '</p>' + 
		        	  '<form id="imageStatus" method="POST" action="/image_changeStatus">' +
	                  		'<input class="hidden" type="text" name="status" value="' + id + '">' + 
					   '</form>'
						 	
						'<form id="imageDelete" method="POST" action="/image_delete">' + 
							'<input class="hidden" type="text" name="delete" value="' + id + '">' + 
			                '<input class="hidden" type="text" name="pathToDelete" value="' + galleryReference + '/' + title+ '">' + 
		        		'</form>'
		}else{
			output = ''
		}
